...
function bindNav(){
  ...
  document.getElementById('roleSel').onchange = (e)=>{
    ...
    const avMap = { rop:'ВС', cur0:'АС', cur1:'РМ', cur2:'ГВ', adm:'КИ' };
...
function renderHome(){
  return `<div class="form-pad">
    <h2 style="margin:8px 0 4px;font-size:18px">Институт Движения · CRM-модуль «Маркетинг»</h2>
    <div style="color:#666;margin-bottom:18px">Начальная страница · 14.05.2026 · ${ST.openTabs.length} активных вкладок</div>
    ...
    <div class="group"><div class="group-h"><span class="ch">▼</span>Избранное</div>
      <div style="padding:10px 14px;display:grid;grid-template-columns:repeat(3,1fr);gap:8px">
        ${['deals','kanban','morning','tasks','report'].map(s=>`
      ...
      <tbody>
        ${list.map(p=>{
          ...
          return `<tr class="${ST.selectedDealId===p.id?'sel':''}${od?' overdue':''}" onclick="selectDeal('${p.id}',event)" ondblclick="openDealCard('${p.id}')">
            ...
            <td>${p.date} 10:14</td>
            <td><a class="lk" onclick="event.stopPropagation();openDealCard('${p.id}')">${p.name}</a></td>
      ...
      ${byCur.map(({c,list})=>`
        <div class="lane">
          <div class="lane-h">
            ...
            <span>${c.full}</span>
            <span class="pill p-gray" style="margin-left:auto">${list.length} пациентов</span>
          </div>
...
let CARD_STATE = { vedType:'', vedSub:'', nextDate:'', format:'', comment:'' };
function renderCard(){
  ...
  if (!CARD_STATE._loaded || CARD_STATE._pid!==p.id){
    CARD_STATE = {_loaded:true,_pid:p.id, vedType:p.vedenie||'', vedSub:'', nextDate:p.nextContact||'', format:p.format||'', comment:''};
  }
  const subs = VED_SUB[CARD_STATE.vedType] || [];
  const filledCount = ['vedType','vedSub','nextDate','format','comment'].filter(k=>CARD_STATE[k]&&CARD_STATE[k].trim()).length;
  const canClose = filledCount===5;
  ...
  <div class="form-pad">
    ...
    <div class="group">
      <div class="group-h"><span class="ch">▼</span>Атрибуты пациента</div>
      <div class="group-b">
        <div class="l">Ответственный куратор:</div>
        <div class="inp"><select><option value="">— не назначен —</option>${CURATORS.map(c=>`<option ${p.curatorId===c.id?'selected':''}>${c.full}</option>`).join('')}</select><div class="b">↗</div></div>
    ...
    <div class="vedenie-wrap">
      ...
      ${!canClose ? `<div class="err" style="margin-top:4px">⚠ Заполнено ${filledCount} из 5 обязательных полей. Кнопка «Записать и закрыть» станет активной после заполнения всех полей.</div>` : ''}
    ...
    <div class="cpane ${ST.cardTab==='strat'?'show':''}">
      ...
      <div class="strat">
        <div class="strat-card"><div class="day">День 0 · сразу</div><div class="ttl">Пациент не записался → Звонок куратора</div>
          <div class="desc">Цель: согласовать дату ведения, получить устное подтверждение. Приоритет — <b>Высокий</b>. Шаблон скрипта: <code>day0_script_v3</code>.</div></div>
        <div class="strat-card d3"><div class="day">День 3 · нет ответа</div><div class="ttl">Нет ответа → WhatsApp</div>
          <div class="desc">Шаблон сообщения <code>day3_sms</code> с подборкой ближайших слотов. Приоритет — <b>Средний</b>.</div></div>
        <div class="strat-card d7"><div class="day">День 7 · эскалация</div><div class="ttl">Звонок РОП</div>
          <div class="desc">Передача на руководителя отделения. Видно в разделе «Задачи кураторов» с фильтром <b>«Просроченные»</b>.</div></div>
      </div>
...
function applyStrategy(pid){
  ...
  const base = [
    ...
    {d:7, urg:'overdue', type:'Эскалация', title:`Эскалация РОП: ${p.name} не отвечает 7 дней`, tags:['день 7','эскалация','звонок']},
  ];
  base.forEach((b,i)=>{
    ST.tasks.unshift({ id:'t'+(ST.tasks.length+i), curatorId:p.curatorId ?? 0, title:b.title,
      patient:p.name, pid:p.id, meta:`14.05.26 ${10+i*2}:00`, urgency:b.urg, done:false, type:b.type,
      tags:b.tags, status:b.urg==='overdue'?'Просрочена':'Открыта' });
  });
  renderAll();
  toast(`Создано 3 задачи: «${p.name}»`,'ok');
}
...
function renderAll(){
  ...
  let html = '';
  const screens = ['home','deals','kanban','morning','tasks','report','card','taskCard','orderCard'];
  const renderers = { home:renderHome, deals:renderDeals, kanban:renderKanban, morning:renderMorning, tasks:renderTasks, report:renderReport, card:renderCard, taskCard:renderTaskCard, orderCard:renderOrderCard };
  screens.forEach(s=>{
